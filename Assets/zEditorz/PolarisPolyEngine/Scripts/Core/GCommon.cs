...
namespace Pinwheel.Griffin
{
    ...
    public static class GCommon
    {
        ...
        public const string DISCORD = "https://discord.gg/t7msDNK";
        ...
        public const float MAX_TREE_DISTANCE = 5000;
        public const float MAX_GRASS_DISTANCE = 500;
        public const float MAX_COLLIDER_BUDGET = 100;
        ...
        public static GRenderPipelineType CurrentRenderPipeline
        {
            get
            {
                string pipeline = Shader.globalRenderPipeline;
                if (pipeline.Equals("UniversalPipeline,LightweightPipeline"))
                {
                    return GRenderPipelineType.Universal;
                }
                else if (Shader.globalRenderPipeline.Equals("LightweightPipeline"))
                {
                    return GRenderPipelineType.Lightweight;
                }
                else
                {
        ...
        {
            Texture2D t = new Texture2D(width, height, TextureFormat.ARGB32, false, true);
