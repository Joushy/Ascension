...
{ 
    public static partial class GEditorCommon
    {
        ...
        public static int TreeSetSelectionGrid(int groupId, int selectedIndex)
        {
            List<GTreePrototypeGroup> treeSet = GetTreeSetOnActiveTerrainGroup(groupId);
            GTreePrototypeGroup set = null;
            if (treeSet.Count == 0 || (treeSet.Count == 1 && treeSet[0] == null))
            {
                EditorGUILayout.LabelField("No Tree Set found. Assign a Tree Prototype Group to the terrain or terrain group.", WordWrapItalicLabel);
                return selectedIndex;
            }
            else if (treeSet.Count > 1)
            {
                EditorGUILayout.LabelField("Tree Set config in the current terrain group is not consistent.\nTaking the largest set for preview!", WordWrapItalicLabel);
                treeSet.RemoveAll(s => s == null);
                treeSet.Sort((s0, s1) => { return -s0.Prototypes.Count.CompareTo(s1.Prototypes.Count); });
                set = treeSet[0];
            }
            else
            {
            ...
            if (set == GGriffinSettings.Instance.TerrainDataDefault.Foliage.Trees)
            {
                EditorGUILayout.LabelField("This is a demo Tree set, consider creating your own one!", WordWrapItalicLabel);
            }

            GUI.enabled = false;
            EditorGUILayout.ObjectField("Trees", set, typeof(GTreePrototypeGroup), false);
            GUI.enabled = true;
            ...
            if (set != null && set.Prototypes.Count > 0)
            {
                ...
                args.itemSize = GEditorCommon.selectionGridTileSizeSmall;
                args.itemPerRow = 4;
                args.drawPreviewFunction = DrawTreePreview;
                selectedIndex = GEditorCommon.SelectionGrid(args);
            }
            else
            {
                EditorGUILayout.LabelField("No Tree found in this set!", ItalicLabel);
            }
        ...
        public static int GrassSetSelectionGrid(int groupId, int selectedIndex)
        {
            List<GGrassPrototypeGroup> grassSet = GetGrassSetOnActiveTerrainGroup(groupId);
            GGrassPrototypeGroup set = null;
            if (grassSet.Count == 0 || (grassSet.Count == 1 && grassSet[0] == null))
            {
                EditorGUILayout.LabelField("No Grass Set found. Assign a Grass Prototype Group to the terrain or terrain group!", WordWrapItalicLabel);
                return selectedIndex;
            }
            else if (grassSet.Count > 1)
            {
                EditorGUILayout.LabelField("Grass Set config in the current terrain group is not consistent.\nTaking the largest set for preview!", WordWrapItalicLabel);
                grassSet.RemoveAll(s => s == null);
                grassSet.Sort((s0, s1) => { return -s0.Prototypes.Count.CompareTo(s1.Prototypes.Count); });
                set = grassSet[0];
            }
            else
            {
            ...
            if (set == GGriffinSettings.Instance.TerrainDataDefault.Foliage.Grasses)
            {
                EditorGUILayout.LabelField("This is a demo Grass set, consider creating your own one!", WordWrapItalicLabel);
            }

            GUI.enabled = false;
            EditorGUILayout.ObjectField("Grasses", set, typeof(GGrassPrototypeGroup), false);
            GUI.enabled = true;
            ...
            if (set != null && set.Prototypes.Count > 0)
            {
                ...
                args.itemSize = GEditorCommon.selectionGridTileSizeSmall;
                args.itemPerRow = 4;
                args.drawPreviewFunction = DrawGrassPreview;
                selectedIndex = GEditorCommon.SelectionGrid(args);
            }
            else
            {
                EditorGUILayout.LabelField("No Grass found in this set!", ItalicLabel);
            }
        ...
        public static void DrawBackupHelpBox()
        {
            ...
            {
                string label = "Tip";
                string id = "backup-help-box";
                Foldout(label, true, id, () =>
                {
                    EditorGUILayout.LabelField(
                       "Backup System is a great way to manage your Editing History!",
                       WordWrapItalicLabel);
                    if (GUILayout.Button("Open Backup Editor"))
                    {
        ...
        public static string PolygonProcessorSelector(string label, string current)
        {
            List<string> labels = new List<string>();
            labels.Add("None");
            for (int i = 0; i < GGeometry.PolygonProcessorTypeCollection.Count; ++i)
            {
                Type t = GGeometry.PolygonProcessorTypeCollection[i];
                labels.Add(GetClassDisplayName(t));
            }

            int selectedIndex = 1 + GGeometry.PolygonProcessorTypeCollection.FindIndex(t => t.FullName.Equals(current));
            selectedIndex = EditorGUILayout.Popup(label, selectedIndex, labels.ToArray());

            if (selectedIndex >= 1 && selectedIndex <= GGeometry.PolygonProcessorTypeCollection.Count)
            {
                current = GGeometry.PolygonProcessorTypeCollection[selectedIndex - 1].FullName;
            }
            else
            {
