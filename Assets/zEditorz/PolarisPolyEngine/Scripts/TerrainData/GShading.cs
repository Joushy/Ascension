...
namespace Pinwheel.Griffin
{
    public class GShading : ScriptableObject
    {
        ...
        public Texture2D Internal_AlbedoMap
        {
            get
            {
                if (albedoMap == null)
                {
                    ...
                    GCommon.AddObjectToAsset(albedoMap, TerrainData);
                }
        ...
        [SerializeField]
        private Texture2D internal_MetallicMap;
        public Texture2D Internal_MetallicMap
        {
            get
            {
                if (internal_MetallicMap == null)
                {
                    internal_MetallicMap = GCommon.CreateTexture(MetallicMapResolution, Color.clear);
                    internal_MetallicMap.filterMode = FilterMode.Bilinear;
                    internal_MetallicMap.wrapMode = TextureWrapMode.Clamp;
                    internal_MetallicMap.name = METALLIC_MAP_NAME; ;
                    GCommon.AddObjectToAsset(internal_MetallicMap, TerrainData);
                }
                return internal_MetallicMap;
            }
        }
        ...
        public Texture2D Internal_ColorByHeightMap
        {
            get
            {
        ...
        public Texture2D Internal_ColorByNormalMap
        {
            get
            {
        ...
        public Texture2D Internal_ColorBlendMap
        {
            get
            {
        ...
        public Texture2D[] Internal_SplatControls
        {
            get
            {
        ...
        {
            Reset();
            GCommon.FillTexture(Internal_AlbedoMap, Color.clear);
            GCommon.FillTexture(Internal_MetallicMap, Color.clear);
            UpdateLookupTextures();
        ...
        public void UpdateMaterials()
        {
            if (MaterialToRender != null)
            {
                ...
                {
                    MaterialToRender.SetTexture(AlbedoMapPropertyName, Internal_AlbedoMap);
                }
                if (MaterialToRender.HasProperty(MetallicMapPropertyName))
                {
                    MaterialToRender.SetTexture(MetallicMapPropertyName, Internal_MetallicMap);
                }
                if (MaterialToRender.HasProperty(ColorByHeightPropertyName))
                {
                    MaterialToRender.SetTexture(ColorByHeightPropertyName, Internal_ColorByHeightMap);
                }
                if (MaterialToRender.HasProperty(ColorByNormalPropertyName))
                {
                    MaterialToRender.SetTexture(ColorByNormalPropertyName, Internal_ColorByNormalMap);
                }
                if (MaterialToRender.HasProperty(ColorBlendPropertyName))
                {
                    MaterialToRender.SetTexture(ColorBlendPropertyName, Internal_ColorBlendMap);
                }
                ...
                for (int i = 0; i < SplatControlMapCount; ++i)
                {
                    ...
                    {
                        MaterialToRender.SetTexture(SplatControlMapPropertyName + i, Internal_GetSplatControl(i));
                    }
                }
        ...
        private void ResizeMetallicMap()
        {
            ...
            GCommon.CopyToRT(internal_MetallicMap, rt);
            GCommon.CopyFromRT(tmp, rt);
            rt.Release();
            Object.DestroyImmediate(rt);

            tmp.name = internal_MetallicMap.name;
            tmp.filterMode = internal_MetallicMap.filterMode;
            tmp.wrapMode = internal_MetallicMap.wrapMode;
            Object.DestroyImmediate(internal_MetallicMap, true);
            internal_MetallicMap = tmp;
            GCommon.AddObjectToAsset(internal_MetallicMap, TerrainData);
            UpdateMaterials();
        }
        ...
        public void UpdateLookupTextures()
        {
            ...
            GCommon.AddObjectToAsset(colorByHeightMap, TerrainData);

            colorByNormalMap.SetPixels(cbnColors);
            colorByNormalMap.Apply();
            GCommon.AddObjectToAsset(colorByNormalMap, TerrainData);

            colorBlendMap.SetPixels(cbColors);
            colorBlendMap.Apply();
            GCommon.AddObjectToAsset(colorBlendMap, TerrainData);
        }

        public Texture2D Internal_GetSplatControl(int index)
        {
            if (index < 0 || index >= SplatControlMapCount)
                throw new System.ArgumentException("Index must be >=0 and <=SplatControlMapCount");
            else
            {
                Texture2D t = Internal_SplatControls[index];
                if (t == null)
                {
                    ...
                    GCommon.AddObjectToAsset(t, TerrainData);
                    Internal_SplatControls[index] = t;
                }
                return t;
            }
        }
        ...
        private void ResizeSplatControlMaps()
        {
            for (int i = 0; i < SplatControlMapCount; ++i)
            {
                Texture2D t = Internal_GetSplatControl(i);
                if (t == null)
                    return;
                ...
                Internal_SplatControls[i] = tmp;
                GCommon.AddObjectToAsset(tmp, TerrainData);
            }
        ...
        public void ConvertSplatsToAlbedo()
        {
            ...
            for (int i = 0; i < SplatControlMapCount; ++i)
            {
                Texture2D controlMap = Internal_GetSplatControl(i);
            ...
            GCommon.CopyFromRT(Internal_AlbedoMap, albedoRt);
            albedoRt.Release();
